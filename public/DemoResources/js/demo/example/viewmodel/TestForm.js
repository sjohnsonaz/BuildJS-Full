...
Build('demo.example.viewmodel.TestForm', [ 'build::build.form.Form', 'build::build.ui.element.Paragraph', 'build::build.ui.element.Div', 'build::build.form.input.Text', 'build::build.form.input.CheckBox', 'build::build.binding.TextBinding',
		'build::build.binding.ValueBinding', 'build::build.binding.IfBinding', 'build::build.binding.ForEachBinding', 'demo::demo.example.viewmodel.TestViewModel' ], function(define, $super) {
	define({
		$extends : 'build.form.Form',
		/**
		 * @constructor
		 */
		$constructor : function TestForm(data) {
			$super(this)({
				testValue : {},
				testTrue : {},
				testArray : {}
			...
			var testViewModel = new demo.example.viewmodel.TestViewModel(data);
